...
macro_rules! ftp_stream {
    ($self:expr, $s:ident => $body:expr) => {{
        let kind = $self.stream.as_mut().ok_or_else(|| anyhow::anyhow!("FTP session not connected"))?;
        match kind {
...
impl FtpClient {
    ...
    pub async fn connect(config: &FtpConfig) -> Result<Self> {
        ...
        let mut stream_kind = if config.ftps_enabled {
            ...
            // Keep certificate validation enabled by default. A future UI may
        ...
        // Never probe EPSV by issuing the command directly: EPSV allocates a
        // passive data listener, and abandoning that listener can block the
        // next transfer until the server's data timeout expires. Use EPSV for
        // literal IPv6 endpoints and PASV with the control peer substituted
        // for IPv4/DNS endpoints (the most interoperable NAT-safe default).
        let use_extended_passive =
            config.host.parse::<std::net::IpAddr>().is_ok_and(|address| address.is_ipv6());
        match &mut stream_kind {
            FtpStreamKind::Plain(s) if use_extended_passive => s.set_mode(Mode::ExtendedPassive),
            FtpStreamKind::Secure(s) if use_extended_passive => s.set_mode(Mode::ExtendedPassive),
            FtpStreamKind::Plain(s) => s.set_passive_nat_workaround(true),
            FtpStreamKind::Secure(s) => s.set_passive_nat_workaround(true),
        }
        tracing::info!(
            "FTP data connections will use {}",
            if use_extended_passive { "EPSV" } else { "PASV with NAT workaround" }
        );
        ...
        // Set binary transfer type
        {
            match &mut stream_kind {
                ...
                FtpStreamKind::Secure(s) => s.transfer_type(suppaftp::types::FileType::Binary).await,
            }
...
fn parse_ftp_list_line(line: &str) -> Option<FileEntry> {
    ...
    let size_idx = tokens[..month_idx].iter().rposition(|t| t.parse::<u64>().is_ok());
    let size = size_idx.and_then(|i| tokens[i].parse::<u64>().ok()).unwrap_or(0);
...
fn is_perms_token(s: &str) -> bool {
    ...
    bytes[1..10].iter().all(|&b| matches!(b, b'r' | b'w' | b'x' | b'-'))
}
...
mod tests {
    ...
    async fn test_ftp_connect_and_disconnect() {
        ...
        let mut client = FtpClient::connect(&cfg).await.expect("FTP connect should succeed");
    ...
    async fn test_ftp_crud_cycle() {
        ...
        // 4a. Create directory
        client.create_dir(test_dir).await.expect("create_dir should succeed");
        ...
        assert_eq!(downloaded_data, upload_content, "downloaded content should match uploaded content");
        eprintln!("Download verified: {} bytes match", downloaded_bytes);

        // 4e. Rename the file
        client.rename(&test_file_remote, &renamed_file_remote).await.expect("rename should succeed");
        ...
        client.delete_dir(test_dir).await.expect("delete_dir should succeed");
    ...
    fn test_parse_ftp_list_line_selinux_context_file_year() {
        ...
        let line = "-rw-r--r--. 1 root root system_u:object_r:default_t 85234 Nov 09  2000 gamelist.xml";
        let entry = parse_ftp_list_line(line).expect("should parse");
    ...
    fn test_parse_ftp_list_line_no_group_busybox() {
        ...
        assert!(entry.modified.is_some(), "modified should be parsed, got None");
    }
    ...
    fn test_parse_ftp_list_line_symlink_with_extra_columns() {
        let line = "lrwxrwxrwx. 1 root root system_u:object_r:default_t 10 Mar 01 00:00 link -> target";
        let entry = parse_ftp_list_line(line).expect("should parse");
        assert_eq!(entry.name, "link");
        assert!(matches!(entry.file_type, FileEntryType::Symlink));
    }
    ...
    fn test_parse_ftp_list_line_name_with_spaces_extra_columns() {
        ...
        let line = "-rw-r--r--. 1 root root system_u:object_r:default_t 100 Dec 25 23:59 my file name.txt";
        let entry = parse_ftp_list_line(line).expect("should parse");
        assert_eq!(entry.name, "my file name.txt");
