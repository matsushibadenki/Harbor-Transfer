...
- [Next] 同期プレビューからの安全な実行、除外パターン、実行ログを実装
- [Later] 同期、追加ストレージプロトコル、配布対応を段階的に実装
...
次は同期プレビューで実行対象と競合方針を選択できるようにし、停止可能な同期ジョブと安全な実行ログへ進む。
