...
- [Later] Google Drive接続をOAuth 2.0認証、共有ドライブ対応、API制限への安全な再試行とともに統合
...
## Phase 9 — Google Drive [Later]

**完了条件:** Google Drive APIへ安全に認証し、マイドライブと共有ドライブで閲覧・転送・基本ファイル操作を利用できる。

- [Later] OAuth 2.0 Authorization Code + PKCEの認証設計、リダイレクト方式、必要最小限のスコープを確定
- [Later] refresh tokenをmacOS Keychainへ保存し、ブックマークにはアカウント表示名と接続識別子だけを保持
- [Later] マイドライブ、共有アイテム、共有ドライブを選択できる接続設定とブックマークUIを3言語で追加
- [Later] Google Driveのfile IDを基準にしたアダプタを`RemoteFileSystem`へ統合し、同名項目と複数親を安全に扱うパス変換を実装
- [Later] 一覧のページネーション、検索、フォルダ作成、改名、移動、ゴミ箱への削除、ダウンロードを実装
- [Later] resumable upload、進捗、停止、取消、再試行、APIレート制限の指数バックオフを転送キューへ統合
- [Later] Googleドキュメント形式は直接上書きせず、エクスポート形式と再アップロード時の制約をUIで明示
- [Later] 共有ドライブ権限、ショートカット、容量制限、変更競合を含む統合テストを追加
