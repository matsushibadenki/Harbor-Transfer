...
- [Done] `0.2.5`で左右ペインの可変幅、ローカル側パンくず、ツールバー配置・高さ・配色の統一、狭いウインドウでの表示崩れ修正を統合
...
- [Done] 各ペインから別のブックマーク接続を独立して開き、接続状態・パス・検索・選択をペインごとに保持できるようにした
- [Done] デュアルペイン間のドラッグ、コピー、移動を、ローカル／任意の接続先で共通利用できる操作へ統合
- [Done] 左右の同名フォルダを再帰比較し、差分項目だけを選んで安全に転送できる比較ビューを実装
- [Later] 高度なSSH認証、再帰検索、Quick Look、追加クラウド、自動化を段階的に実装
...
- [Done] 中央の転送操作領域をドラッグ可能なスプリッターとし、左右ペイン幅の保存、キーボード調整、中央位置への復元に対応
- [Done] ローカル／リモートの操作列、パス列、接続状態列、ファイル表示領域、パンくず列の高さと配置を共通化し、双方からパスをコピー可能にする
- [Done] 左右それぞれから別のブックマーク接続を独立して開き、接続状態、パス、検索、選択をペイン単位で保持。接続先変更と左右入れ替えを独立した操作として実装
- [Done] ローカル↔リモート、ローカル↔ローカル、リモート↔リモートのドラッグ、コピー、移動を共通操作へ統合。複数選択、双方向ボタン、通常ドロップでコピー、Shift＋ドロップで移動へ対応
- [Done] 異なる接続先間の転送をアプリ専用一時キャッシュ経由で実行し、転送先の完了後だけ移動元を削除。同名項目は既存データを消さず重複しない名称へ変更
- [Done] 左右で開いているフォルダを再取得してサイズ・更新日時を比較するビューを追加。左のみ／右のみ／差分／一致／判定保留を表示し、差分項目を選んで既存データを保護しながら双方向にコピー可能にする
- [Done] 同名の通常ファイルを選択的にSHA-256で照合可能にする。リモート側は照合時のみ一時取得し、照合後にキャッシュを削除する
- [Later] 対応プロトコル間のserver-side copyを能力判定し、一時キャッシュ転送を最適化
- [Done] フォルダ内部を相対パスで再帰比較し、入れ子になった差分ファイルを選択して転送可能にする。転送先の不足している親フォルダは安全性を検査してから作成する
- [Next] 比較ビューで同名ファイルの競合を上書き／別名保存／スキップから選択できるようにし、実行前に変更内容を再確認する
